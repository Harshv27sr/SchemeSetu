      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-3 bg-slate-50/95 dark:bg-slate-900 border border-slate-200/60 dark:border-white/5 rounded-3xl p-6 shadow-sm dark:shadow-none space-y-6">
          <div className="flex justify-between items-center pb-4 border-b border-slate-200/50 dark:border-white/5">
            ...
            <button
              onClick={handleReset}
              className="text-xs text-govblue-600 dark:text-govblue-400 hover:text-govblue-700 dark:hover:text-govblue-300 font-bold flex items-center space-x-0.5 cursor-pointer transition-all"
            >
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-700 dark:text-slate-300">Citizen Focus Group</label>
            <div className="flex flex-wrap gap-1.5">
              {FILTER_CATEGORIES.map(cat => (
                <button
                  key={cat}
                  onClick={() => setCategoryFilter(cat)}
                  className={`px-3 py-1.5 border rounded-xl text-[11px] font-bold transition-all cursor-pointer ${categoryFilter === cat
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-700 dark:text-slate-300">Residency Region</label>
            <select
              value={stateFilter}
              onChange={(e) => setStateFilter(e.target.value)}
              className="form-input py-2 text-xs cursor-pointer"
            >
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-700 dark:text-slate-300">Social Category (Caste)</label>
            <select
              value={casteFilter}
        ...
        <div className="lg:col-span-9 space-y-6">
          ...
          {filteredSchemes.length === 0 ? (
            <div className="bg-slate-50/70 dark:bg-slate-900 border border-slate-200/60 dark:border-white/5 rounded-3xl p-12 text-center text-slate-500 dark:text-slate-400 space-y-2">
              ...
              <p className="text-xs text-slate-400 dark:text-slate-500">Try resetting keyword strings or checking "All States / Central" categories.</p>
              <button
                onClick={handleReset}
                className="mt-4 px-4 py-2 bg-govblue-50 dark:bg-govblue-950/20 text-govblue-600 dark:text-govblue-400 rounded-xl text-xs font-bold cursor-pointer hover:bg-govblue-100 dark:hover:bg-govblue-900/30 transition-all"
              >
                  ...
                  >
                    <div>
                      ...
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex flex-wrap gap-1">
                          <span className="text-[9px] font-bold text-govblue-700 dark:text-govblue-400 bg-govblue-50 dark:bg-govblue-950/30 px-2 py-0.5 rounded-full uppercase">
                            {scheme.state === "Central" ? "Central Scheme" : `${scheme.state} State`}
                          </span>
                          <span className="text-[9px] font-bold text-govgreen-700 dark:text-govgreen-450 bg-govgreen-50 dark:bg-govgreen-950/20 px-2 py-0.5 rounded-full uppercase">
                        ...
                        {user?.profile?.fullName && (
                          <div className={`flex items-center space-x-1 text-[10px] font-bold px-2 py-0.5 rounded-full ${el.score === 100
                      ...
                      <h3 className="text-base font-extrabold text-slate-800 dark:text-white leading-tight mb-2 hover:text-govblue-600 dark:hover:text-govblue-400 transition-all">
                        <Link to={`/schemes/${scheme._id}`}>{scheme.title}</Link>
                      </h3>
                      <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-3 leading-relaxed mb-6">{scheme.description}</p>
                    </div>
                    ...
                    <div className="border-t border-slate-100 dark:border-white/5 pt-4 mt-2 flex items-center justify-between">
                      ...
                      <Link
                        ...
                        className="px-4.5 py-2 text-xs font-bold text-govblue-600 dark:text-govblue-400 hover:text-white bg-slate-50 dark:bg-slate-800 hover:bg-govblue-600 dark:hover:bg-govblue-600 rounded-xl transition-all flex items-center cursor-pointer"
                      >
